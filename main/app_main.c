...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        msg_id = esp_mqtt_client_publish(client, "/topic/door/unlock", "Closed", 0, 1, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);

        msg_id = esp_mqtt_client_subscribe(client, "/topic/door/status", 0);
...
void door_unlock_request(){
    while (1)
    {
        if (!gpio_get_level(GPIO_NUM_0))
        {
            msg_id = esp_mqtt_client_publish(client, "/topic/door/unlock", "Open", 0, 1, 0);
            ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
            vTaskDelay(200);
        }
    }
...
void app_main(void)
{
    ...
    xTaskCreate(door_unlock_request, "door_unlock_request", 2048, NULL, 5, NULL);
